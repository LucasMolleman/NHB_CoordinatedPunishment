...
(bar mean_type xaxis if pun_type==4, barwidth(0.8)  fcolor(gs6)  lcolor(black) lwidth(medthick) ), ///
	...
	ytitle("Percentage", size(medlarge)) xtitle("") xscale(range(0.45 3.55))  legend(off) plotregion(margin(b = 0)  lcolor(black)) ///
...
collapse (count) x, by(anger pun_type)
drop if anger==.
gen percent = .
replace percent = (x/2285)*100 if pun_type==1
replace percent = (x/188)*100 if pun_type==2
replace percent = (x/245)*100 if pun_type==3
replace percent = (x/146)*100 if pun_type==4

* Panel A
tw (bar percent anger if pun_type==2, barwidth(1)  fcolor(maroon)  lcolor(black) lwidth(medthin) ///
ylabel(0(10)60, nogrid labsize(medlarge)) ytitle("Percentage", size(medlarge)) xlabel(1(1)7, nogrid labsize(medlarge)) xtitle("") ///
subtitle("Independent" "punishers ({it:n} = 188)", box bexpand alignment(middle) fcolor(sand*0.6))  ///
...
name(one, replace)  fxsize(44))

* Panel B
tw (bar percent anger if pun_type==3, barwidth(1)  fcolor(green)  lcolor(black) lwidth(medthin) ///
ylabel(none) yscale(range(0 60)) ytitle("",) xlabel(1(1)7, nogrid labsize(medlarge)) xtitle("") ///
subtitle("Coordinated" "punishers ({it:n} = 245)", box bexpand alignment(middle) fcolor(sand*0.6))  ///
...
name(two, replace)  )
*graphregion(ilcolor(gs6))

* Panel C
tw (bar percent anger if pun_type==4, barwidth(1)  fcolor(gs6)  lcolor(black) lwidth(medthin) ///
ylabel("") yscale(range(0 60)) ytitle("", size(medlarge)) xlabel(1(1)7, nogrid labsize(medlarge)) xtitle("") ///
subtitle("Anti-coordinated" "punishers ({it:n} = 146)", box bexpand alignment(middle) fcolor(sand*0.6))  ///
...
* Panel D
tw (bar percent anger if pun_type==1, barwidth(1)  fcolor(white)  lcolor(black) lwidth(medthin) ///
ylabel("") yscale(range(0 60)) ytitle("", size(medlarge)) xlabel(1(1)7, nogrid labsize(medlarge)) xtitle("") ///
subtitle("Non-punishers" "({it:n} = 2,285)", box bexpand alignment(middle) fcolor(sand*0.6))  ///
...
(bar mean_ip coop_type, barwidth(0.8)  fcolor(maroon)  lcolor(black) lwidth(medthick) ) , ///
	...
	ytitle("Percentage", size(medsmall)) xtitle("") xscale(range(0.45 3.55))   plotregion(margin(zero) ) ///
...
(bar mean_type_case xaxis if pun_type==4 & case==1, barwidth(0.8)  fcolor(gs6)  lcolor(black)  ), ///
	xlabel("", noticks labgap(1) labsize(small)) yscale(range(0 0.6)) fxsize(72) fysize(44)  ///
...
(bar mean_type_case xaxis if pun_type==4 & case==3, barwidth(0.8)  fcolor(gs6)  lcolor(black)  ), fxsize(72) ///
...
graph combine sup_fig1a sup_fig1b sup_fig1c sup_fig1d, imargin(vsmall) l1title("Percentage")
graph export "sup_fig1.pdf", replace
graph export "sup_fig1.png", replace
...
(bar mean_type xaxis if pun_type==4 & unconditional_punishment==0, barwidth(0.8)  fcolor(gs6)  lcolor(black) lwidth(medthick) ), ///
	...
	ytitle("Percentage", size(medlarge)) xtitle("") xscale(range(0.45 3.55)) plotregion(margin(b = 0) lcolor(black))   ///
...
graph export "sup_fig2.png", replace
