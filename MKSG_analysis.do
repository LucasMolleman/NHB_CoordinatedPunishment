...
(bar mean_type xaxis if pun_type==4, barwidth(0.8)  fcolor(gs6)  lcolor(black) lwidth(medthick) ), ///
	...
	text(0.645 0.5 "{bf:a}", size(medlarge)) name(fig_2a, replace) subtitle("All punishers", box bexpand alignment(middle) fcolor(sand*0.6)) fxsize(55)	
...
(bar mean_type2 xaxis if pun_type==4 & unconditional_punishment==0, barwidth(0.8)  fcolor(gs6)  lcolor(black) lwidth(medthick) ), ///
	...
	text(0.13 3.0 "25.9%", box fcolor(white) lcolor(white) size(medsmall)) ///
	text(0.645 0.5 "{bf:b}", size(medlarge))
...
(bar mean_type2 xaxis if pun_type==4 & unconditional_punishment==1, barwidth(0.8)  fcolor(gs6)  lcolor(black) lwidth(medthick)  ), ///
	...
	text(0.14 3.0 "27.7%", box fcolor(white) lcolor(white) size(medsmall)) 	///
	text(0.645 0.5 "{bf:c}", size(medlarge))
...
yscale(range(0 60)) fxsize(43) name(one, replace) ///
...
* Panel B
tw hist anger if pun_type==3, discrete percent fcolor(green)  plotregion(lcolor(black)) lcolor(black) ///
...
(bar mean_ip coop_type, barwidth(0.8)  fcolor(maroon)  lcolor(black) lwidth(medthick) ) , ///
	...
	ylabel(0  .2 "20"  .4 "40"  .6 "60" .8 "80"  1 "100"  , angle(0) nogrid labsize(medlarge)) yscale(range(0 1)) 	///
	ytitle("Percent", size(medlarge)) xtitle("") xscale(range(0.45 3.55))   plotregion(margin(zero) ) ///
...
graph export "sup_fig1.emf", replace
...
graph export "sup_fig2.emf", replace
...
* Show Regression Output
esttab m1 m2 m3,  star(* 0.05 ** 0.01 *** 0.001) b(%9.3f) se(%9.3f) varwidth(25) modelwidth(10)
